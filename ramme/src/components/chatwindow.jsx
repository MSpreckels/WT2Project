...
class ChatWindow extends Component {
    ...
    return (
      <div>
        <div
          ...
          style={{ height: "250px", overflowY: "scroll", scrollTop: "100" }}
        >
          {GetMessages(this.props.messages).map(x => (
            <p key={x[0]} className={x[3] ? "myMessage" : "otherMessage"}>
              {x[1] + ": " + x[2]}
            </p>
          ))}
        </div>
        <div>
          <input
            id="chatMessage"
            type="input"
            onKeyDown={e => InvokeButtonClickOnEnter(e)}
          />
          <button
  ...
  scrollToBottom() {
    ...
    chatMessageContainer.scrollTop =
      chatMessageContainer.scrollHeight - chatMessageContainer.clientHeight;
  }
}
